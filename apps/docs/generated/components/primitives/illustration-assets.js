...
function normalizeDensity(density) {
    return density && validDensities.has(density) ? density : undefined;
}
...
export function createIllustrationAsset({ id, src, darkSrc = "", alt = "", purpose = "decorative", source = "open-doodles", format, density, theme = "auto", fallbackText, loading = "lazy", } = {}) {
    ...
    if (id)
        figure.dataset.assetId = id;
    const resolvedTheme = theme === "dark" || theme === "light" ? theme : "auto";
    figure.dataset.theme = resolvedTheme;
    const assetModel = {
        id: id ?? null,
        src: src ?? null,
    ...
    if (status === "ready") {
        ...
        const handleImageError = () => {
            ...
            media.replaceWith(createFallback(fallbackText ?? "Illustration unavailable"));
        ...
        figure.append(createFallback(fallbackText ?? "Illustration unavailable"));
    }
    return {
        assetNode: figure,
        assetModel,
