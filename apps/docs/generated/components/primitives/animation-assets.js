...
export function resolveAnimationRuntime(runtime = globalThis?.lottie) {
...
export function createAnimationAsset({
  label,
  source = "",
  animationData,
  renderer = "svg",
  ...
  fallbackText = "Static animation fallback",
  reducedMotion = prefersReducedAnimation(),
} = {}) {
  ...
  if (canPlay) {
    ...
    if (resolvedState === "paused" && typeof animation.pause === "function") animation.pause();
    if (resolvedState === "complete" && typeof animation.goToAndStop === "function") animation.goToAndStop(100, true);
