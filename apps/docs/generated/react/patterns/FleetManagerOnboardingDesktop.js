...
function resolveState({ disabled, permissionBlocked, blocked, validating, complete, empty, inProgress, state, tasks, reviewRows, }) {
    ...
    if (complete || state === "complete")
        return "complete";
    if (empty || (!tasks.length && !reviewRows.length) || state === "empty")
        return "empty";
    if (inProgress || tasks.some((task) => task.checked) || state === "in-progress")
        return "in-progress";
    return state ?? "not-started";
}
export const FleetManagerOnboardingDesktop = forwardRef(function FleetManagerOnboardingDesktop({ label = "Fleet manager onboarding", description, density, state, disabled = false, inProgress = false, blocked = false, validating = false, complete = false, empty = false, permissionBlocked = false, steps = [], currentStep = 0, metrics = [], tasks = [], fields = [], selects = [], reviewColumns, reviewRows = [], settings, validation, primaryAction, secondaryAction, emptyState, feedback, className = "", onTaskChange, onAction, ...rest }, ref) {
    ...
    return React.createElement("div", {
        ...
        : null, resolvedState === "empty" || resolvedState === "permission-blocked"
