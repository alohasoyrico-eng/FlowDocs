...
function resolveState({ disabled, loading, error, selectedTicketKey, detailOpen, alerts, state }) {
    ...
    if (detailOpen || state === "detail-open")
        return "detail-open";
    if (selectedTicketKey || state === "ticket-selected")
        return "ticket-selected";
    if (alerts?.open || state === "alerts-open")
        return "alerts-open";
...
function surfaceStateFor(resolvedState) {
    ...
    if (resolvedState === "loading")
        return "sunken";
    if (resolvedState === "detail-open" || resolvedState === "ticket-selected" || resolvedState === "alerts-open")
        return "selected";
    return "default";
}
function summaryTone(summary, resolvedState) {
    ...
    if (resolvedState === "error")
        return "danger";
    if (resolvedState === "alerts-open")
        return "warning";
    if (resolvedState === "detail-open" || resolvedState === "ticket-selected")
        return "info";
    return "neutral";
}
export const TicketQueue = forwardRef(function TicketQueue({ label = "Ticket queue", description, density = "sm", state, disabled = false, loading = false, error, selectedTicketKey, detailOpen = false, summaries = [], alerts, tickets = {}, detail, feedback, className = "", onAlertOpenChange, onAlertSelect, onAlertDismiss, onAlertMarkAll, onTicketSearchChange, onTicketFilterRemove, onTicketFiltersReset, onTicketSortChange, onTicketSelect, onTicketPageChange, onTicketBulkAction, onTicketToolbarOverflowSelect, onDetailOpenChange, onDetailAction, onFeedbackAction, ...rest }, ref) {
    const normalizedSummaries = normalizeArray(summaries).filter((summary) => Boolean(summary?.label));
