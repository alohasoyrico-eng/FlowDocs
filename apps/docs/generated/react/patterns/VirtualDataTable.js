...
export const VirtualDataTable = forwardRef(function VirtualDataTable({
  label,
  description,
  density,
  state,
    ...
    ? [
      {
        ...
        render: (row) => {
          ...
          return React.createElement(Checkbox, {
            label: `${selection.rowLabel ?? "Select row"} ${row.label ?? key}`,
            checked,
  ...
  return React.createElement(
    ...
    normalizedRows.length && resolvedState !== "loading" && resolvedState !== "error"
      ...
      : null,
    bulkActions.filter((action) => action?.label).map((action) => React.createElement(Button, {
      ...action,
