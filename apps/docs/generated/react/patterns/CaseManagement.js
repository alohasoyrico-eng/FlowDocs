...
function resolveState({ disabled, loading, error, selectedCaseKey, detailOpen, filters, timeline, state }) {
    ...
    if (detailOpen || state === "detail-open")
        return "detail-open";
    if (selectedCaseKey || state === "case-selected")
        return "case-selected";
    if (filters?.open || filters?.dirty || state === "filters-open")
        return "filters-open";
    if (timeline?.filtered || state === "activity-filtered")
...
function summaryTone(summary, resolvedState) {
    ...
    if (resolvedState === "error")
        return "danger";
    if (resolvedState === "filters-open" || resolvedState === "activity-filtered")
        return "warning";
    if (resolvedState === "case-selected" || resolvedState === "detail-open")
        return "info";
    return "neutral";
}
function isCaseManagementSummary(summary) {
...
export const CaseManagement = forwardRef(function CaseManagement({ label = "Case management", description, density = "sm", state, disabled = false, loading = false, error, selectedCaseKey, detailOpen = false, summaries = [], filters, cases = {}, detail, timeline, feedback, className = "", onFilterDrawerOpenChange, onFilterApply, onFilterReset, onSavedFilterSelect, onCaseSearchChange, onCaseFilterRemove, onCaseFiltersReset, onCaseSortChange, onCaseSelect, onCasePageChange, onCaseBulkAction, onCaseToolbarOverflowSelect, onDetailOpenChange, onDetailAction, onTimelineEventSelect, onTimelineFilterRemove, onTimelineClear, onFeedbackAction, ...rest }, ref) {
    ...
    return React.createElement(Surface, {
        ...
        : null, React.createElement(DenseOperationalList, {
        ...cases,
