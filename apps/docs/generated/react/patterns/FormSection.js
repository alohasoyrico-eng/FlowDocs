...
function fieldState(sectionState, field) {
    if (field.disabled)
        return "disabled";
    if (field.loading || sectionState === "validating" || sectionState === "saving")
        return "loading";
    if (field.error || sectionState === "invalid")
        return "error";
    return field.state;
}
function selectionState(sectionState, field, selected) {
    if (field.disabled || sectionState === "disabled")
        return "disabled";
    if (field.error || sectionState === "invalid")
        return "error";
...
export const FormSection = forwardRef(function FormSection({ title, description, density, state, disabled = false, loading = false, fields = [], primaryAction, secondaryAction, validation, feedback, onAction, onFieldValueChange, className = "", ...rest }, ref) {
    ...
    }, React.createElement("h3", null, title), description ? React.createElement("p", null, description) : null, validation?.summary ? React.createElement("p", null, validation.summary) : null, normalizedFields.map((field) => {
        ...
        if (field.kind === "radio-button") {
            ...
            return React.createElement(RadioButton, {
                ...
                onCheckedChange: (checkedValue, meta, event) => {
                    ...
                    if (checkedValue)
                        onFieldValueChange?.(fieldKey, meta.value, meta, event);
                },
            });
        }
