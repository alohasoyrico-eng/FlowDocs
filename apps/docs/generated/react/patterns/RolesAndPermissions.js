...
function normalizeRoles(roles) {
  return (Array.isArray(roles) ? roles : []).filter((role) => role?.label);
}

function normalizePermissions(permissions) {
  return (Array.isArray(permissions) ? permissions : []).filter((permission) => permission?.label);
}
...
function cellControl({ mode, checked, disabled, density, role, permission, onPermissionChange }) {
  ...
  const shared = {
    ...
    checked,
    disabled,
    name: `${roleKey(role)}-${permissionKey(permission)}`,
    "data-role-key": roleKey(role),
    "data-permission-key": permissionKey(permission),
  };
  const onCheckedChange = (nextChecked, meta, event) => {
    onPermissionChange?.(roleKey(role), permissionKey(permission), nextChecked, { role, permission, meta }, event);
  };
...
export const RolesAndPermissions = forwardRef(function RolesAndPermissions({
