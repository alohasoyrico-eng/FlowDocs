...
function fieldStateFor(resolvedState, field = {}) {
  if (resolvedState === "disabled" || field.disabled) return "disabled";
  if (resolvedState === "loading" || field.loading) return "loading";
  if (field.error || field.state === "error") return "error";
  if (field.state) return field.state;
  if (field.value) return "filled";
  return "default";
}
