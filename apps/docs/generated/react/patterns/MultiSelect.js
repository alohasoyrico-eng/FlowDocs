...
function resolveState({ disabled, loading, open, selectedCount, optionsCount, state }) {
  ...
  if (selectedCount) return "selected";
  return state ?? "closed";
}
...
export const MultiSelect = forwardRef(function MultiSelect({
  ...
  return React.createElement(
    ...
    normalizedOptions.length
      ? normalizedOptions.map((option) => {
        ...
        return React.createElement(Checkbox, {
          ...
          onCheckedChange: (nextChecked, meta, event) => updateValue(valueKey, nextChecked, event),
        });
      })
    ...
    overflowCount
      ...
      : null,
    clearAction?.label && selectedCount
