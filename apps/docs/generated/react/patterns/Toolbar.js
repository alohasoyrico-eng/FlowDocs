...
function resolveState({ disabled, loading, permissionBlocked, filters, overflow, dense, state }) {
    ...
    if (overflow || state === "overflow")
        return "overflow";
    if ((filters?.length ?? 0) > 0 || state === "filter-active")
...
export const Toolbar = forwardRef(function Toolbar({ label = "Local actions", density, state, dense = false, loading = false, disabled = false, permissionBlocked = false, search, actions = [], filters = [], badges = [], overflow, feedback, topbar, className = "", ...rest }, ref) {
