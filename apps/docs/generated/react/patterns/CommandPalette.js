...
function normalizeCommands(commands, executingKey) {
    return (Array.isArray(commands) ? commands : [])
        .filter((command) => Boolean(command?.label))
        .map((command) => {
...
function resolveState({ open, query, loading, executingKey, commands, state, }) {
    if (executingKey || state === "executing")
        return "executing";
    if (loading || state === "loading")
        return "loading";
    if (!open && !state)
        return "closed";
    if (!commands.length && query)
        return "empty";
...
export const CommandPalette = forwardRef(function CommandPalette({ label = "Command palette", description, triggerLabel, closeLabel = "Close command palette", query = "", placeholder = "Search commands", density, state, open = false, loading = false, commands = [], selectedKey, executingKey, empty, feedback, primaryAction, onOpenChange, onQueryChange, onCommandSelect, onPrimaryAction, className = "", ...rest }, ref) {
    ...
    return React.createElement("div", {
        ...
        : null, !normalizedCommands.length && resolvedState === "empty"
