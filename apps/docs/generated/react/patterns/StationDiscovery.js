...
const validStationStates = new Set(["default", "hover", "focus", "selected", "unavailable", "disabled"]);
const validStationVariants = new Set(["fuel", "ev", "service", "cluster"]);
const validListStates = new Set(["default", "hover", "selected", "loading", "error", "disabled"]);
function sanitizeRestProps(rest) {
    return Object.fromEntries(Object.entries(rest).filter(([key]) => key.startsWith("data-") || key.startsWith("aria-")));
}
function toStationVariant(variant) {
    return validStationVariants.has(variant) ? variant : "fuel";
}
function toStationState(state) {
    return validStationStates.has(state) ? state : undefined;
}
function toListState(state) {
    return validListStates.has(state) ? state : undefined;
}
function normalizeStations(stations) {
    return (Array.isArray(stations) ? stations : [])
        .filter((station) => Boolean(station?.label))
        .map((station, index) => {
        const normalized = {
        ...
        if (station.state)
            normalized.state = station.state;
        if (station.icon)
            normalized.icon = station.icon;
        return normalized;
    });
}
function normalizeFallbackList(fallbackList) {
    if (!fallbackList)
        return undefined;
    return {
...
function toMapPinInput(station, selected) {
    const pin = {
        id: station.id,
        label: station.label,
        value: station.value,
    ...
    if (station.state)
        pin.state = station.state;
    return pin;
}
function toSearchResult(station) {
    const result = {
        key: station.key,
        label: station.label,
        meta: station.meta,
...
function resolveState({ disabled, loading, error, permissionState, runtimeStatus, stations, selectedStation, route, state, }) {
    ...
    if (permissionState === "denied")
        return "denied";
    if (runtimeStatus === "runtimeUnavailable")
        return "runtimeUnavailable";
    if (runtimeStatus === "providerMissing")
...
export const StationDiscovery = forwardRef(function StationDiscovery({ label = "Station discovery", description = "", density, state, disabled = false, loading = false, permission = "prompt", center = null, stations = [], selectedStation, selectedStationKey, route = null, fallbackList, runtime = null, mapStyle = null, tileProvider = null, query, search, emptyState, error, errorPanel, action, onStationSelect, onRouteAction, onQueryChange, onSubmit, onAction, className = "", ...rest }, ref) {
    ...
    if (normalizedFallbackList)
        mapOptions.fallbackList = normalizedFallbackList;
    ...
    const searchResults = search?.results ?? normalizedStations.map(toSearchResult);
    ...
    return React.createElement("div", {
        ...
        onResultSelect: (key, event) => {
            ...
            if (station)
                onStationSelect?.(station.id, station, event);
        },
        onSubmit,
        ...
        : null, mapPrimitive.routeSummary
