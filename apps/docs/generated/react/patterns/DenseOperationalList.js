...
function resolveState({ disabled, loading, error, rows, selectedKeys, filters, state, }) {
    ...
    if (error || state === "error")
        return "error";
    if (selectedKeys.length > 0 || state === "selected")
        return "selected";
    if (filters.length > 0 || state === "filtered")
        return "filtered";
...
function surfaceStateFor(resolvedState) {
    if (resolvedState === "disabled")
        return "disabled";
    if (resolvedState === "selected" || resolvedState === "filtered")
        return "selected";
...
export const DenseOperationalList = forwardRef(function DenseOperationalList({ label = "Dense operational list", description, density = "sm", state, disabled = false, loading = false, error, search, filters = [], toolbar, table = {}, bulkActions, feedback, resultCount, selectedKeys = [], className = "", onSearchChange, onFilterRemove, onFiltersReset, onSortChange, onRowSelect, onPageChange, onBulkAction, onToolbarOverflowSelect, onFeedbackAction, ...rest }, ref) {
    const rows = normalizeArray(table.rows);
    const activeFilters = normalizeArray(filters).filter((filter) => Boolean(filter?.label));
    ...
    return React.createElement(Surface, {
        ...
        : null, React.createElement(FilterChipGroup, {
        label: `${label} filters`,
