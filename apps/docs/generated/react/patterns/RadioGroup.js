...
function notifyValueChange(onValueChange, nextValue, meta, event) {
    onValueChange?.(nextValue, meta, event);
}
export const RadioGroup = forwardRef(function RadioGroup({ label, helper, density, state, disabled = false, loading = false, required = false, name, options = [], value, defaultValue = "", clearLabel, applyAction, validation, className = "", onValueChange, onApply, onClear, ...rest }, ref) {
    const reactId = useId();
    ...
    return React.createElement(Surface, {
    ...
    }, React.createElement("div", { "data-flow-slot": "question" }, React.createElement("h3", null, label), helper ? React.createElement("p", null, helper) : null), normalizedOptions.map((option) => {
        ...
        ? React.createElement("div", { "data-flow-slot": "actions" }, clearLabel ? React.createElement(Button, { label: clearLabel, variant: "ghost", density, disabled: isDisabled || !currentValue, onClick: clear }) : null, applyAction?.label
