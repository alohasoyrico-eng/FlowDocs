...
export const AuthenticationLoginBiometricsAndOtp = forwardRef(function AuthenticationLoginBiometricsAndOtp({
  ...
  return React.createElement(
    ...
    React.createElement(Surface, {
      surfaceRole: "section",
      state: isBlocked ? "disabled" : isBusy ? "raised" : resolvedState === "recovered" ? "selected" : "default",
      density,
    ...
    showOtp
      ? React.createElement(CodeInput, {
        ...
        state: otp?.state ?? (resolvedState === "otp-invalid" ? "error" : isDisabled ? "disabled" : otp?.value?.length >= (otp?.length ?? 6) ? "complete" : "default"),
        error: otp?.error ?? (resolvedState === "otp-invalid" ? "Code could not be verified." : undefined),
        disabled: isDisabled || otp?.disabled,
      })
      : null,
    showBiometric
