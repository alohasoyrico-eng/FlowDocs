...
export const AdvancedFilters = forwardRef(function AdvancedFilters({
  ...
  const normalizedApplied = (Array.isArray(appliedFilters) ? appliedFilters : []).filter((filter) => filter?.label);
  const invalid = Boolean(validation?.message || normalizedFields.some((field) => field.error || field.invalid));
