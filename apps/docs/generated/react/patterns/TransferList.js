...
function resolveState({ sourceCount, targetCount, selectedCount, transferring, partial, invalid, disabled, state, }) {
    ...
    if (partial || state === "partial")
        return "partial";
    if (transferring || state === "transferring")
        return "transferring";
    if (sourceCount === 0 || state === "empty-source")
        return "empty-source";
    if (targetCount === 0 || state === "empty-target")
        return "empty-target";
    if (selectedCount > 0 || state === "selecting")
        return "selecting";
    return state ?? "idle";
}
...
function toListItem(item, index, isDisabled) {
    ...
    if (item.valueLabel !== undefined)
        listItem.value = item.valueLabel;
    if (item.icon !== undefined)
        listItem.icon = item.icon;
    if (item.tone !== undefined)
        listItem.tone = item.tone;
    if (isDisabled || item.disabled !== undefined)
...
export const TransferList = forwardRef(function TransferList({ label = "Transfer list", density, state, disabled = false, transferring = false, partial = false, invalid = false, sourceLabel = "Available", targetLabel = "Selected", source = [], target = [], selectedSourceKeys = [], selectedTargetKeys = [], search, filterInput, multiSelect, moveToTargetAction, moveToSourceAction, validation, feedback, className = "", onSourceSelect, onTargetSelect, onItemCheckedChange, ...rest }, ref) {
    ...
    return React.createElement("div", {
        ...
        : null, React.createElement(Badge, {
    ...
    }), sourceItems.map((item, index) => renderCheckbox(item, index, density, isDisabled, "source", onItemCheckedChange)), React.createElement(Button, {
        ...(moveToTargetAction ?? {}),
    ...
    }), targetItems.map((item, index) => renderCheckbox(item, index, density, isDisabled, "target", onItemCheckedChange)), validation
