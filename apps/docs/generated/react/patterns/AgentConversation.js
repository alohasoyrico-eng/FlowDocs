...
function normalizeState({ state, disabled, sending, offline, error, handoff, composer }) {
    ...
    if (handoff?.active)
        return "handoff";
    if (sending || composer?.sending)
        return "sending";
    if (state && validStates.has(state))
        return state;
    if (composer?.value || composer?.defaultValue)
        return "composing";
    return "active";
}
function surfaceStateFor(state) {
    ...
    if (state === "error" || state === "offline")
        return "sunken";
    if (state === "handoff" || state === "sending")
        return "selected";
    return "default";
}
...
export const AgentConversation = forwardRef(function AgentConversation({ label = "Agent conversation", description, density = "md", state, disabled = false, sending = false, offline = false, error, thread = {}, composer, handoff, feedback, selectedMessageKey, className = "", onMessageAction, onComposerChange, onSend, onAttach, onHandoffAction, onFeedbackAction, ...rest }, ref) {
