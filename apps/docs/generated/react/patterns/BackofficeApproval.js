...
function docRecordValue(record) {
    return record[docToken];
}
function docRecordKey(record) {
    return String(record.key ?? record.id ?? record.label ?? docRecordValue(record));
}
function hasDocRecordLabel(record) {
    return Boolean((record ? record[docToken] : undefined) || record?.doc || record?.label);
}
...
function resolveState({ disabled, loading, error, deciding, selectedDocumentKey, detailOpen, pendingCount, state, }) {
    ...
    if (loading || state === "loading")
        return "loading";
    if (deciding || state === "deciding")
        return "deciding";
    if (detailOpen || state === "detail-open")
        return "detail-open";
    if (selectedDocumentKey || state === docSelectedState)
        return docSelectedState;
    if (pendingCount > 0 || state === "pending-review")
...
function summaryTone(summary, resolvedState) {
    ...
    if (resolvedState === "error")
        return "danger";
    if (resolvedState === "pending-review" || resolvedState === "deciding")
        return "warning";
    if (resolvedState === "detail-open" || resolvedState === docSelectedState)
        return "info";
    return "neutral";
}
export const BackofficeApproval = forwardRef(function BackofficeApproval({ label = "Backoffice approval", description, density = "sm", state, disabled = false, loading = false, deciding = false, error, selectedDocumentKey, detailOpen = false, summaries = [], documents = [], queue = {}, detail, feedback, className = "", onDocumentSearchChange, onDocumentFilterRemove, onDocumentFiltersReset, onDocumentSortChange, onDocumentSelect, onDocumentPageChange, onDocumentBulkAction, onDetailOpenChange, onDetailAction, onApprove, onReject, onFeedbackAction, ...rest }, ref) {
