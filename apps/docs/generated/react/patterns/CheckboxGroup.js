...
export const CheckboxGroup = forwardRef(function CheckboxGroup({
  ...
  const updateOption = (nextValue, checked, event) => {
    ...
    commitValue([...nextValues], { value: nextValue, checked }, event);
  };

  const toggleAll = (checked, meta, event) => {
    const nextValues = new Set(selectedValues);
    if (checked) enabledValues.forEach((item) => nextValues.add(item));
    else enabledValues.forEach((item) => nextValues.delete(item));
    commitValue([...nextValues], { value: "__all", checked, indeterminate: false }, event);
  };
  ...
  return React.createElement(
    ...
    normalizedOptions.map((option) => {
      ...
      return React.createElement(Checkbox, {
        ...
        onCheckedChange: (checked, meta, event) => updateOption(nextValue, checked, event),
      });
    }),
    validation?.message
    ...
    clearLabel || applyAction?.label
      ? React.createElement(
        ...
        clearLabel ? React.createElement(Button, { label: clearLabel, variant: "ghost", density, disabled: isDisabled || selectedValues.size === 0, onClick: clear }) : null,
        applyAction?.label
