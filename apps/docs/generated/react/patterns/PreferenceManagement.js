...
function normalizeState({ state, disabled, permissionBlocked, saving, dirty, dangerZone, sections, settings }) {
  ...
  if (permissionBlocked || settings?.permissionBlocked) return "permission-blocked";
  if (dangerZone?.open || state === "danger-confirming") return "danger-confirming";
  if (saving || settings?.saving || sections.some((section) => section.loading || section.state === "saving")) return "saving";
  if (state === "invalid" || settings?.state === "invalid" || sections.some((section) => section.state === "invalid")) return "invalid";
  if (state && validStates.has(state)) return state;
  if (dirty || settings?.dirty || sections.some((section) => section.state === "dirty")) return "dirty";
  return "idle";
}

function surfaceStateFor(state) {
  if (state === "disabled" || state === "permission-blocked") return "disabled";
  if (state === "invalid" || state === "danger-confirming") return "selected";
  if (state === "saving") return "raised";
  return "default";
}

function badgeToneFor(state) {
  if (state === "invalid" || state === "danger-confirming") return "danger";
  if (state === "dirty" || state === "permission-blocked") return "warning";
...
export const PreferenceManagement = forwardRef(function PreferenceManagement({
  ...
  onDangerConfirm,
  onDangerCancel,
  onDangerRecoveryAction,
  ...rest
}, ref) {
  const normalizedSections = normalizeArray(sections).filter((section) => section?.title);
