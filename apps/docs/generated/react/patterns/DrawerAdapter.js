...
function cardKey(card, index) {
  return card.key ?? card.title ?? `card-${index}`;
}

export const DrawerAdapter = forwardRef(function DrawerAdapter({
  ...
  const normalizedCards = (Array.isArray(cards) ? cards : []).filter((card) => card?.title);
  const normalizedActions = (Array.isArray(actions) ? actions : []).filter((action) => action?.label);
  const listItems = (Array.isArray(list?.items) ? list.items : []).filter((item) => item?.label);
