...
function surfaceStateForTemplate(state) {
    if (state === "disabled")
        return "disabled";
    if (state === "error" || state === "offline" || state === "permission")
        return "raised";
    return "default";
}
...
function composerStateForTemplate(state, sending) {
    if (state === "disabled" || state === "permission" || state === "offline")
        return "disabled";
    if (sending || state === "loading")
        return "sending";
    if (state === "error")
        return "error";
    return undefined;
...
export const AgentWorkspace = forwardRef(function AgentWorkspace({ label = "Agent workspace", description = "Resolve driver conversations with context, handoff, and recovery.", density = "md", tone, state, disabled = false, loading = false, error = false, permissionBlocked = false, offline = false, handoff = false, selectedConversation, defaultSelectedConversation = "handoff", onSelectedConversationChange, topbar, conversations = defaultConversations, thread, composer, feedback, context, className = "", ...rest }, ref) {
    ...
    const handleConversationSelect = (conversation, event) => {
        ...
        if (selectedConversation === undefined)
            setInternalSelectedConversation(key);
    ...
    }), description ? React.createElement("p", { id: "agent-workspace-description", "data-template-module": "workspace-context" }, description) : null, React.createElement(Surface, {
        surfaceRole: "section",
        state: surfaceStateForTemplate(resolvedState),
        density,
        elevation: "raised",
