...
function surfaceStateForTemplate(state) {
  ...
  if (state === "loading") return "sunken";
  return "default";
}
...
function patternStateForTemplate(state) {
  ...
  if (state === "permission") return "disabled";
  return undefined;
}
...
export const InternalOperationsConsole = forwardRef(function InternalOperationsConsole({
  label = "Internal operations console",
  description = "Coordinate cases, tickets, accounts, pricing, approvals, and growth operations.",
  density = "sm",
  tone,
  state,
  disabled = false,
  ...
  const handleRouteSelect = (key, route, event) => {
    ...
    if (selectedModule === undefined) setInternalSelectedModule(key);
