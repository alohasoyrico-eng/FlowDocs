...
export const FleetManagerDesktop = forwardRef(function FleetManagerDesktop({ label = "Fleet manager desktop", description = "Scan operational health, isolate exceptions, and assign follow-up.", density = "md", tone, state, disabled = false, loading = false, error = false, permissionBlocked = false, offline = false, selectedDashboard, defaultSelectedDashboard = "overview", onSelectedDashboardChange, drawerOpen, defaultDrawerOpen = false, onDrawerOpenChange, topbar, sidebar, rolesAndPermissions, metrics = defaultMetrics, exceptions = defaultExceptions, activity, filters, className = "", ...rest }, ref) {
    ...
    }, `${metric.label}: ${metric.value} ${metric.detail ?? ""}`))), React.createElement(Surface, {
        surfaceRole: "panel",
    ...
    }, `${exception.label}: ${exception.owner ?? "Unassigned"} ${exception.age ?? ""}`))), React.createElement(RolesAndPermissions, {
        ...(rolesAndPermissions ?? {}),
