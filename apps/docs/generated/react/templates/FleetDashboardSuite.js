...
export const FleetDashboardSuite = forwardRef(function FleetDashboardSuite({ label = "Fleet dashboard suite", description = "Move across operational dashboards while preserving filters, thresholds, and drill-down evidence.", density = "md", tone, state, disabled = false, loading = false, error = false, permissionBlocked = false, offline = false, selectedDashboard, defaultSelectedDashboard = "overview", onSelectedDashboardChange, drawerOpen, defaultDrawerOpen = false, onDrawerOpenChange, topbar, sidebar, filters = defaultFilters, kpis = defaultKpis, drillDownRows = defaultRows, chartSummary, financeVisible = true, className = "", ...rest }, ref) {
    ...
    const handleRouteSelect = (key, route, event) => {
        ...
        if (selectedDashboard === undefined)
            setInternalSelectedDashboard(key);
    ...
    }, route.label))), React.createElement(Surface, {
        surfaceRole: "panel",
        state: surfaceStateForTemplate(resolvedState),
        density,
        elevation: "raised",
    ...
    }, `${filter.label}: ${filter.value}`))), React.createElement(Surface, {
        surfaceRole: "panel",
    ...
    }, `${kpi.label}: ${kpi.value} ${kpi.definition ?? ""}`)), chartSummary ? React.createElement("span", { "data-chart-summary": "true" }, chartSummary) : null), React.createElement(Surface, {
        surfaceRole: "panel",
    ...
    }, resolvedState === "error" ? React.createElement("span", { "data-state": "error", "data-template-recovery": "drill-down-error" }, "Drill-down evidence unavailable") : null, drillDownRows.map((row) => React.createElement("span", {
