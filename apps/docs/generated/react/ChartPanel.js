...
function renderBullet(values, labels) {
    ...
    return points.map((point) => {
        const value = point.value;
        const pointLabel = point.label;
        return React.createElement("span", { key: point.key, className: "chart-panel__bullet", role: pointLabel ? "listitem" : undefined, tabIndex: pointLabel ? 0 : undefined, "data-tooltip": pointLabel ? `${pointLabel}: ${value}` : undefined }, pointLabel ? React.createElement("b", null, pointLabel) : null, React.createElement("progress", { className: "chart-panel__bullet-meter", max, value, tabIndex: -1, "aria-hidden": "true" }), React.createElement("em", null, String(value)));
    });
}
...
function renderPareto(values, labels) {
    ...
    return React.createElement("svg", { className: "chart-panel__svg chart-panel__pareto-svg", viewBox: "0 0 160 72", role: "img", "aria-hidden": "true" }, sorted.map((item, index) => React.createElement("rect", {
        key: item.key,
...
function renderPlot(type, values, labels, series, comparisons, segments) {
    ...
    if (type === "bullet")
        return renderBullet(values, labels);
    if (type === "comparison")
        return renderComparison(comparisons, values, labels);
    if (type === "pareto")
        return renderPareto(values, labels);
    return renderLinePlot(values, labels, type, series);
}
export const ChartPanel = forwardRef(function ChartPanel({ label, value = "", caption = "", values, valueLabels, labels, segments, series, comparisons, variant = "sparkline", state = "default", tone = "neutral", density, fullWidth = false, className = "", ...rest }, ref) {
    ...
    }, React.createElement("header", { className: "chart-panel__header" }, React.createElement("div", null, label ? React.createElement("strong", null, label) : null, caption ? React.createElement("p", null, caption) : null), value ? React.createElement("output", null, value) : null), React.createElement("figure", { role: "group", "aria-label": chartPrimitive.textSummary }, React.createElement("div", { className: "chart-panel__plot", role: "list" }, renderPlot(normalizeVariant(chartPrimitive.type), resolvedValues, resolvedLabels, resolvedSeries, resolvedComparisons, resolvedSegments)), React.createElement("span", { className: "chart-panel__tooltip", role: "status", "aria-live": "polite", "data-visible": "false" }), React.createElement("div", { className: "chart-panel__echarts", "aria-hidden": "true" }), React.createElement("script", { type: "application/json", className: "chart-panel__option" }, JSON.stringify(optionModel))));
});
