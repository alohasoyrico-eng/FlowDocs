...
function formatSliderValue({ value, initialValue, valueLabel, unit, formatValue }) {
    ...
    if (valueLabel && String(value) === String(initialValue))
        return valueLabel;
    return unit ? `${value}${unit}` : String(value);
}
export const Slider = forwardRef(function Slider({ label, value, min = 0, max = 100, step = 1, variant = "continuous", state = "default", density, unit, disabled = false, name = "", valueLabel, formatValue, onValueChange, className = "", ...rest }, ref) {
    ...
    const formattedValue = useMemo(() => formatSliderValue({ value: currentValue, initialValue: initialValueRef.current, valueLabel, unit, formatValue }), [currentValue, formatValue, unit, valueLabel]);
    ...
    return React.createElement("label", {
    ...
    }, React.createElement("span", { className: "slider__meta" }, React.createElement("span", { className: "slider__label" }, label), React.createElement("output", { className: "slider__value", "data-slider-output": "" }, formattedValue)), React.createElement("span", { className: "slider__control" }, React.createElement("input", {
        ...flowRestProps(rest),
        ref,
        type: "range",
        ...
        onBlur: handleBlur,
    }), React.createElement("span", { className: "slider__track", "aria-hidden": "true" }), React.createElement("span", { className: "slider__fill", "aria-hidden": "true" }), React.createElement("span", { className: "slider__thumb", "aria-hidden": "true" })));
});
