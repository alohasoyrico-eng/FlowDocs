...
export const SegmentedControl = forwardRef(function SegmentedControl({ label, items, selectedKey, onValueChange, variant = "outlined", density, className = "", id, ...rest }, ref) {
    ...
    if (!label || !normalizedItems.length)
        return null;
    const commitKey = (nextKey, restoreFocus = false, event) => {
        ...
        if (restoreFocus)
            schedule(() => itemRefs.current.get(nextKey)?.focus());
    };
    ...
    return React.createElement("div", {
        ...
        }, selected ? React.createElement("span", { className: "segmented-control__indicator", "aria-hidden": "true" }) : null, icon
            ? React.createElement("span", { className: "segmented-control__icon", "aria-hidden": "true" }, icon)
            : null, React.createElement("span", { className: "segmented-control__label", "aria-hidden": iconOnly ? "true" : undefined }, itemLabel));
    }));
});
